...
import com.mirror.authservice.domain.recovery.OtpService;
import com.mirror.authservice.infrastructure.email.EmailService;

import com.mirror.authservice.domain.auth.AuthResponse;
...
import com.mirror.authservice.domain.auth.RefreshTokenRepository;
...
import com.mirror.authservice.infrastructure.security.JwtUtil;
...
public class AuthService {
    ...
    public com.mirror.authservice.domain.user.UserResponse adminCreateUser(String username, String email, String rawPassword, Role role) {
        if (userRepository.existsByEmail(email)) {
            throw new RuntimeException("Email is already registered!");
        }
        if (userRepository.existsByUsername(username)) {
    ...
    public java.util.List<com.mirror.authservice.domain.user.UserResponse> getAllUsers() {
        return userRepository.findAll().stream().map(this::mapToUserResponse).toList();
    }

    public com.mirror.authservice.domain.user.UserResponse getUserById(UUID id) {
    ...
    @Transactional
    public com.mirror.authservice.domain.user.UserResponse updateUser(UUID id, com.mirror.authservice.domain.user.UserUpdateRequest request) {
        User user = userRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("User not found"));
    ...
    private com.mirror.authservice.domain.user.UserResponse mapToUserResponse(User user) {
        return com.mirror.authservice.domain.user.UserResponse.builder()
                .id(user.getId())
                .username(user.getUsername())
                .email(user.getEmail())
                .role(user.getRole())
